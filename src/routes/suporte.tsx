...
const prioVar = (p: string) =>
  p === "critica" ? "destructive" : p === "alta" ? "default" : "outline";
...
function Suporte() {
  ...
  const submit = () => {
    if (!form.assunto || !form.clienteId) {
      toast.error("Preencha assunto e cliente.");
      return;
    }
    addTicket({
  ...
  return (
    <PageContainer>
      <PageHeader
        title="Suporte"
        description={`${tickets.length} tickets · ${tickets.filter((t) => t.status !== "resolvido").length} em aberto`}
        actions={
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <Select
                      value={form.canal}
                      onValueChange={(v) => setForm({ ...form, canal: v as TicketCanal })}
                    >
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(v) => setForm({ ...form, prioridade: v as TicketPrioridade })}
                    >
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
          ...
          return (
            <Link key={t.id} to="/suporte/$id" params={{ id: t.id }}>
              <Card className="p-4 hover:shadow-md transition-shadow h-full">
                ...
                {t.sla && (
                  <p className="text-[10px] text-muted-foreground mt-2">SLA: {formatDate(t.sla)}</p>
                )}
              </Card>
            </Link>
          );
