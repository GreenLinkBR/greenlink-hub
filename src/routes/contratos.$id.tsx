...
import { ArrowLeft, FileSignature, PauseCircle, PlayCircle, XCircle } from "lucide-react";
import { toast } from "sonner";

export const Route = createFileRoute("/contratos/$id")({
  head: () => ({ meta: [{ title: "Contrato — GreenLink ADM" }] }),
  component: Detalhe,
  notFoundComponent: () => <PageContainer><p className="text-muted-foreground">Contrato não encontrado.</p></PageContainer>,
});

function Detalhe() {
  const { id } = Route.useParams();
  const { contratos, clientes, lancamentos, faturarContrato, updateContrato } = useAppStore();
  ...
  const cronograma = Array.from({ length: 12 }).map((_, i) => {
    const d = new Date(); d.setMonth(d.getMonth() + i); d.setDate(10);
    return { ref: d, valor: ct.valorMensal };
  });

  return (
    <PageContainer>
      <Link to="/contratos" className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground mb-3">
        <ArrowLeft className="h-4 w-4" /> Contratos
      </Link>
      <PageHeader title={ct.numero} description={`${cli?.nome ?? "—"} · ${ct.descricao ?? ""}`}
        actions={<div className="flex gap-2 flex-wrap">
          <Button onClick={() => { faturarContrato(ct.id); toast.success("Fatura gerada no financeiro."); }}>
            <FileSignature className="h-4 w-4 mr-2" />Gerar fatura
          </Button>
          {ct.status === "ativo"
            ? <Button variant="outline" onClick={() => updateContrato(ct.id, { status: "suspenso" })}><PauseCircle className="h-4 w-4 mr-2" />Suspender</Button>
            : <Button variant="outline" onClick={() => updateContrato(ct.id, { status: "ativo" })}><PlayCircle className="h-4 w-4 mr-2" />Reativar</Button>}
          <Button variant="outline" onClick={() => updateContrato(ct.id, { status: "encerrado" })}><XCircle className="h-4 w-4 mr-2" />Encerrar</Button>
        </div>} />
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        <Card className="p-5 lg:col-span-2">
          ...
          <div className="grid sm:grid-cols-2 gap-3 text-sm">
            ...
            <Info label="Valor mensal" value={<span className="font-semibold">{formatBRL(ct.valorMensal)}</span>} />
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-2">
          {cronograma.map((p, i) => (
            <div key={i} className="rounded-lg border p-2 text-center">
              <p className="text-[10px] uppercase text-muted-foreground">{p.ref.toLocaleDateString("pt-BR", { month: "short", year: "2-digit" })}</p>
...
function Info({ label, value }: { label: string; value: React.ReactNode }) {
  return <div><p className="text-xs uppercase text-muted-foreground">{label}</p><div className="mt-0.5">{value}</div></div>;
}
